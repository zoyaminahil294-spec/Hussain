...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#fcfcfd]">
      ...
      <main className="flex-1 pt-24 pb-20">
        {view === ViewMode.ADMIN && currentUser ? (
          <div className="container mx-auto px-6 py-8">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-12 gap-6">
               ...
               <div className="flex gap-2 overflow-x-auto pb-2 w-full md:w-auto no-scrollbar">
                 ...
                 <button onClick={() => setAdminSubTab('deployment')} className={`flex-none px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${adminSubTab === 'deployment' ? 'bg-indigo-600 text-white shadow-xl' : 'bg-white text-slate-400 border border-slate-100'}`}>Go Live 🚀</button>
               </div>
            </div>
              ...
              <div className="grid lg:grid-cols-12 gap-12">
                ...
                <div className="lg:col-span-7">
                  <div className="bg-white rounded-[3rem] border border-slate-100 overflow-hidden shadow-sm p-8">
                    <h3 className="font-black text-slate-900 uppercase tracking-widest text-[11px] mb-8">Stock List ({products.length})</h3>
                    <div className="space-y-4">
                      ...
                      {products.length === 0 && (
                        <div className="text-center py-24">
                          <p className="text-slate-300 font-bold uppercase tracking-widest text-xs">Awaiting Hussain's Products</p>
                        </div>
                      )}
                    </div>
                  </div>
            ...
            {adminSubTab === 'deployment' && (
              <div className="bg-white p-8 md:p-16 rounded-[3rem] border border-slate-100 shadow-xl max-w-4xl mx-auto">
                <div className="flex items-center gap-4 mb-8">
                  ...
                  <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tight">Vercel Deployment Guide</h3>
                </div>
                <div className="space-y-6 text-slate-600 leading-relaxed">
                  <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                    <h4 className="font-black text-slate-900 text-xs uppercase tracking-widest mb-2">1. GitHub</h4>
                    <p className="text-sm">Apna sara code download karke GitHub par upload karein. Vercel automatically GitHub se code utha leta hai.</p>
                  </div>
                  <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                    <h4 className="font-black text-slate-900 text-xs uppercase tracking-widest mb-2">2. Vercel Settings</h4>
                    <p className="text-sm">Vercel Dashboard par ja kar <strong>Environment Variables</strong> mein <code>API_KEY</code> lazmi dalein (Gemini wali key).</p>
                  </div>
                  <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                    <h4 className="font-black text-slate-900 text-xs uppercase tracking-widest mb-2">3. Data Warning</h4>
                    <p className="text-sm">Abhi products aapke browser mein hain. Vercel par nayi site banne ke baad aapko products ek baar dobara add karne honge taake wo permanent ho jayein.</p>
                  </div>
                  <button onClick={() => window.open('https://vercel.com/new', '_blank')} className="w-full bg-indigo-600 text-white font-black py-5 rounded-2xl uppercase text-[10px] tracking-widest shadow-xl hover:bg-indigo-700 transition-all">Open Vercel Dashboard</button>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="space-y-24">
            <section className="mesh-gradient min-h-[60vh] flex items-center justify-center text-center px-6 rounded-b-[4rem] shadow-2xl relative overflow-hidden">
                <div className="relative z-10 max-w-4xl">
                  ...
                  <button onClick={() => document.getElementById('store')?.scrollIntoView({behavior:'smooth'})} className="bg-white text-slate-950 font-black px-12 py-6 rounded-2xl shadow-2xl hover:scale-105 transition-all uppercase text-[10px] tracking-widest">Shop Collection</button>
                </div>
            </section>

            <div id="store" className="container mx-auto px-6">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-16">
                <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tighter">Premium Collection</h3>
                <input type="text" placeholder="Search..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full md:w-96 bg-white border border-slate-200 rounded-2xl py-5 px-8 outline-none font-bold shadow-sm" />
              </div>
              ...
              {filteredProducts.length === 0 ? (
                <div className="py-40 text-center bg-white rounded-[4rem] border border-dashed border-slate-200 flex flex-col items-center">
                  <p className="text-slate-300 font-black uppercase tracking-widest text-xl mb-6">Store is waiting for your products</p>
                  {currentUser ? (
                    <button onClick={() => setView(ViewMode.ADMIN)} className="bg-indigo-600 text-white font-black px-10 py-5 rounded-2xl uppercase text-[10px] tracking-widest shadow-xl">Go to Admin Hub</button>
                  ) : (
                    <button onClick={() => setIsAuthOpen(true)} className="bg-slate-900 text-white font-black px-10 py-5 rounded-2xl uppercase text-[10px] tracking-widest shadow-xl">Owner Login to Add Products</button>
                  )}
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
                  {filteredProducts.map(product => (
                    <div key={product.id} className="innovation-card group bg-white rounded-[3rem] overflow-hidden flex flex-col shadow-sm">
                      <div className="relative aspect-square cursor-pointer" onClick={() => setSelectedImage(product.images[0])}>
      ...
      <footer className="bg-slate-950 text-white py-16 px-6 text-center rounded-t-[4rem]">
        <h2 className="text-2xl font-black uppercase tracking-tighter mb-4">Hussain Innovat</h2>
        <p className="text-slate-500 uppercase tracking-widest text-[9px] font-black">Official Store & Admin Hub</p>
      </footer>
